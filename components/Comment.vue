    ...
    <div class="opration">
      <div class="emoji-panel-btn" @click="showEmojiPanel">
       ...
       <div>
        ...
        <input class="nickname-input" placeholder="请输入邮箱" v-model="email" />
      </div>
    ...
    <div class="comments-list">
      <div class="comments-list-item" v-for="(item,index) in comments" v-bind:key="index">
        <div class="comments-list-item-heading">
          <img src="../assets/img/head_image.jpg" />
          <span class="comments-list-item-username" v-html="item.nickname"></span>
          <span class="comments-list-item-time" v-html="dateFormat(item.createTime)"></span>
        </div>
  ...
  methods: {
    saveComment() {
      ...
      let new_comment = {
        ...
        email: this.email,
        content: this.content.replace(/:.*?:/g, this.emoji), // 替换":"符号包含的字符串,通过emoji方法生成表情<span></span>
        createTime: new Date()
      };
      _this.$axios
